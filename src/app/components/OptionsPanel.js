...
const handleOptions = (e) => {
  ...
  return (
    ...
    <input 
          className="rounded-xl sm:h-8 w-4/5 mt-2 mx-2 font-bold sm:w-full sm:text-xs p-2 mb-4 border-2 border-slate-400 dark:bg-slate-600 dark:text-white focus:outline-none focus:ring focus:ring-slate-600" 
          type='text' 
          name='options' 
          placeholder='Press Enter/Comma for next option'
          onKeyDown={handleTags}
          onChange={handleOptions}/>
    {tags.map((tag,index) => <div key={tag} className="bg-slate-800 flex w-auto p-4 justify-around pt-0.5 h-6 rounded-xl ml-1 mt-2 text-white text-center sm:text-xs sm:h-4">{tag}
